...
export default function TextEditOverlay({ node, stageViewport, onTextChange, onConfirm, onCancel, onFontChange }) {
  ...
  return (
    <>
      ...
      <div
        ref={toolbarRef}
        className="absolute flex items-center gap-1.5 bg-[#2d3139] border border-white/10 rounded-lg shadow-2xl px-2 py-1.5 pointer-events-auto"
        style={{
          left: `${toolbarLeft}px`,
          top: `${screenY}px`,
      ...
      >
        <select
          ...
          className="h-8 px-2 rounded text-sm bg-[#363b44] text-white border border-white/15 cursor-pointer"
        ...
        <button
          onClick={onCancel}
          className="flex items-center gap-1 px-2.5 py-1 rounded text-sm font-medium bg-red-700 hover:bg-red-600 text-white transition-colors whitespace-nowrap"
        >
          <X size={14} /> Cancel
        </button>
        <button
          onClick={onConfirm}
          className="flex items-center gap-1 px-2.5 py-1 rounded text-sm font-medium bg-[#0fff95] text-[#24272f] hover:bg-[#0de882] transition-colors whitespace-nowrap"
        >
          <Check size={14} /> Done
        </button>
      </div>
